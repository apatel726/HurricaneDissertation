...
%JBCOMMENT: Probably worth updating this prior to submission
\hypersetup{
...
%JBCOMMENT: Recommend incorporating a short preview of the results (i.e. which works better, multivariate or univariate) into the last sentence.
\end{abstract}

% begin adding from AMS_Paper_1221

\section{Scope}
%JBCOMMENT: Is "scope" a required section for AMS papers? I'm used to an introductory section, which would expand on the abstract, providing motivational details, a little bit of background, a short description of the approach and a preview of results.
...
%JBCOMMENT: Suggest adding quantitative attribute for wind (speed?)
...
%JBCOMMENT: Output formats seem like a really low-level implementation detail. They can probably be removed unless you feel that they are a really important/novel part of the research.
...
%JBCOMMENT: This seems low for ALL natural disasters. Is it just weather and climate related disasters?
Since 1980, natural disasters have imposed more than \$1.75 trillion in damages; of that total, more than \$927.5 billion were due to the destructive effects of tropical storms [1]. %JBCOMMENT: Need citation
Hurricanes have traditionally been predicted using a traditional forecast called the Dvorak technique [2], a subjective process in which multiple meteorologists assess the same data and arrive at distinct intensity and track predictions. %JBCOMMENT: Need citation
These predictions are based on three main components: dynamics of the current hurricane season, statistical evidence, and an ensemble approach that covers everything. %JBCOMMENT: Unclear what "everything" entails. Is this a combination of seasonal and statistical evidence, or other things as well?
There are drawbacks with each method. The dynamics of the current season do not help early season hurricanes, statistical evidence can be far reaching and might not account for changing features such as climate change, which has increased sea-surface temperatures. The ensemble approach combines the drawbacks of each and amplifies their respective errors. These errors are then accounted for in the ``cone of probability,'' which makes hurricane predictions improbable. %JBCOMMENT: Not clear what's meant my "improbable." Does this mean to indicate that forecast error margins are too high to be of practical utility?

The earliest models are statistical based and the best to date is known as CLIPER5, Climatology and Persistence (CLP5), which uses a multivariate linear regression method [3]. %JBCOMMENT: Are CLIPER5 and CLP5 the same thing? Might be worth deduplicating if the same or clarifying the relationship between them if they're different.
CLIPER5 is commonly identified as an early model as most scientists believe that using statistical model only is not accurate enough to make predictions, this is where a deep learning architecture could help inference and make better predictions to reduce loss and help agencies get limited resources where they will be needed. %JBCOMMENT: Suggest breaking this into two sentences.
Traditional forecast models have a hard time predicting what meteorologists describe as ``intensification,'' which is the increase in sustained wind speeds of 35 miles per hour (mph) over a 24-hour period. This research will lay the groundwork to introduce other variables from satellite measurements, which could increase the accuracy of such models by 200% [4]. %JBCOMMENT: Need citation
...
There are many ML models that can be utilized for the task of hurricane forecasting, including Dynamic Bayesian Networks (DBNs), Hidden Markov Models (HMMs), Recurrent Neural Networks (RNNs), Long Short-Term Memory (LSTM) networks and Bi-Directional Long-Short Term Memory (Bi-LSTM) networks. Each of them have pros and cons; however, for the purposes of this research, we will assess the utility of Bi-LSTM networks. %JBCOMMENT: I would suggest adding a brief description of each of the aforementioned approaches and describe why they are inferior to Bi-LSTMs for this problem.

The pros are that this type of network and architecture is considered state of the art as it is able to keep gated information as has the ability to look in both directions of the time series data (history, and future predictions). %JBCOMMENT: Suggest adding a little more detail on *why* being able to look in both directions is beneficial.
This algorithm type is typical for models that are used in voice recognition and text reading. The primary drawback of Bi-LSTMs is their requirement for significant volumes of training data, which could not be available for some instances, and as a result, it requires a large computational load. %JBCOMMENT: While compute is a concern for processing large datasets, I think the availability of curated, high-quality datasets is the bigger concern, and meshes better with the following sentence about how we're leveraging a pre-curated dataset from NOAA.
For this study, we utilize a curated hurricane dataset provided by the National Oceanic and Atmospheric Administration (NOAA), which provide hurricane records dating back to 1851 for hurricanes. %JBCOMMENT: Recommend replacing "records" with the type of data (e.g., track data and whatever category of data would summarize wind speed, barometric pressure measurements, etc.)
As the numbers of hurricanes grows per year, so does the training dataset which allows for a continuous improvement of the model. Recent research in computational methods include de-featuring imagery into respect RGB pixels and forecast at a 24-hour time lag [5]. %JBCOMMENT: Unclear what the connection is betweent the last sentence regarding imagery and the present research (maybe this is future work). Suggest removal from this section.

\section{Definitions and Acronyms}
%JBCOMMENT: I'm not accustomed to seeing definitions and acronyms within the main body of a conference paper; maybe in an appendix, but typically things are defined and discussed inline fluid with the narrative. I'm guessing this is a carry-over from the design exam, so suggest removal unless this is required per the AMS template.
...
%JBCOMMENT: Not sure this section is necessary; this information would typically be captured as citations in the references section. Low-level implementation details, like specific software dependencies (e.g., NumPy Pandas, TensorFlow, etc.) would normally be ommitted and documented only on the project GitHub repository.
...
%JBCOMMENT: Not sure what this section is necessary or what it is intended to include.
\# TODO
...
\section{Assumptions}
%JBCOMMENT: Most of these things are actually related to experiment setup, suggest renaming this section and crafting the narrative around the data processing steps that were executed to support the ensuing experiments. Docker references and run commands could be removed and saved for the GitHub README.
...
Assumes we use the wind radii and category structure from previous research by Dr. Knaff [7]. %JBCOMMENT: Need citation
...
This code assumes for the incoming data will be in the same csv file format as the NHC and NOAA HURDAT2 database [8]. %JBCOMMENT: Need citation
...
%JBCOMMENT: Most of the discussion regarding system design and ML algorithm would be packaged with the approach/solution description. It would probably also focus less on implementation details (e.g., cloud-hosted virtual machines, Docker, JupyterLab, etc.) and more on the algorithm implementation and the general design for data processing to support ML training.
...
Figure 4: Deep RNN for Hurricane Predictions for Univariate Predictions [9] %JBCOMMENT: Is this a citation for a figure published in another paper? If so, you would likely need permission from the authors if they hold the copyright. If the publisher holds the copyright, you may not need to. If it's just a vanilla RNN, it may be easier to recreate the figure or use one with a creative commons license.
...
%JBCOMMENT: This analysis of alternatives would typically be packaged with the approach and/or experiment setup to justify the decisions that were made (e.g., the model that was selected and the data processing steps that were applied in support of the experiment).
...
Bidirectional RNNs were first produced by Schuster and Paliwal in their journal article title Bidirectional Recurrent Neural Networks [10]. %JBCOMMENT: Need citation
